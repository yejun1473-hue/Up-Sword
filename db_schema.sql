...
drop policy if exists "inv_slots_upsert_own" on public.user_inventory_slots;
create policy "inv_slots_upsert_own" on public.user_inventory_slots
for insert with check (auth.uid() = user_id);
...
create policy "user_missions_upsert_own" on public.user_missions
for insert with check (auth.uid() = user_id);
...
drop policy if exists "user_achievements_upsert_own" on public.user_achievements;
create policy "user_achievements_upsert_own" on public.user_achievements
for insert with check (auth.uid() = user_id);
...
-- Seed basic missions/achievements (safe upserts)
